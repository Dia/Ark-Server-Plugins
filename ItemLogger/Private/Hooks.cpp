...
namespace ItemLogger::Hooks
{
    ...
    void Hook_UPrimalItem_AddToInventory(UPrimalItem* _this, UPrimalInventoryComponent* toInventory, bool bEquipItem, bool AddToSlotItems, FItemNetID* InventoryInsertAfterItemID, bool ShowHUDNotification, bool bDontRecalcSpoilingTime, bool bIgnoreAbsoluteMaxInventory)
    {
        ...
        if (toOwner && !_this->bIsEngram().Get())
        {
            ...
            std::string fullname;
            std::vector<std::string> names = split(full_name.ToString(), ' ');
            fullname = names[0];
            ...
            if (type.ToString() != "Resource" && type.ToString() != "Consumable")
            {
                FString mapname;
                UWorld* world = ArkApi::GetApiUtils().GetWorld();
                world->GetMapName(&mapname);
                const bool res = database->SaveItemLog(Add, sourceSteamId, sourcePlayerName, destSteamId, newOwnerName.ToString(), _this->ItemIDField().ItemID1, _this->ItemIDField().ItemID2, type.ToString(), _this->ItemQuantityField(), (int)_this->ItemQualityIndexField(), _this->bIsBlueprint().Get(), itemName.ToString(), fullname, mapname.ToString());
            }
        }
