...
public:
    explicit MySql(std::string server, std::string username, std::string password, std::string db_name)
    {
        try
        {
            ...
            result = db_.query(fmt::format("CREATE TABLE IF NOT EXISTS ItemLog ("
                ...
                "PRIMARY KEY(id),"
                "INDEX ix_sourceSteamId (sourceSteamId ASC),"
                "INDEX ix_destSteamId (destSteamId ASC),"
    ...
    bool SaveItemLog(uint16 action, uint64 sourceSteamId, std::string sourceName, uint64 destSteamId, std::string destName, uint32 itemId1, uint32 itemId2, std::string type, int quantity, int quality, bool isBlueprint, std::string itemName, std::string blueprintPath, std::string mapname) override
    {
        if (!sourceSteamId)
        {
            sourceSteamId = 0;
        ...
        bool result = db_.query(fmt::format("INSERT INTO ItemLog (action, sourceSteamId, sourceName, destSteamId, destName, itemId1, itemId2, type, quantity, quality, isBlueprint, itemName, blueprintPath, mapname) VALUES ({}, {}, '{}', {}, '{}', {}, {}, '{}', {}, {}, {}, '{}', '{}', '{}');", action, sourceSteamId, sourceName, destSteamId, destName, itemId1, itemId2, type, quantity, quality, isBlueprint, itemName, blueprintPath, mapname));
        if (!result)
        {
